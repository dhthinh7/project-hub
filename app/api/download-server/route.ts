...
export async function GET() {
  try {
    ...
    const exePath = join(process.cwd(), "public", "project-hub-local-server.exe");
    if (existsSync(exePath)) {
      const exeBuffer = readFileSync(exePath);
      return new NextResponse(exeBuffer, {
        headers: {
          "Content-Type": "application/x-msdownload",
          "Content-Disposition": "attachment; filename=project-hub-local-server.exe",
    ...
    const zipPath = join(process.cwd(), "public", "project-hub-local-server.zip");
    if (existsSync(zipPath)) {
      const zipBuffer = readFileSync(zipPath);
      return new NextResponse(zipBuffer, {
        headers: {
          "Content-Type": "application/zip",
          "Content-Disposition": "attachment; filename=project-hub-local-server.zip",
    ...
    const fallbackPackage = {
      name: "project-manager-project-hub-local-server",
      version: "1.0.0",
    ...
    return NextResponse.json(fallbackPackage, {
      headers: {
        ...
        "Content-Disposition": "attachment; filename=project-hub-local-server-instructions.json",
      },
    });
  } catch (error) {
